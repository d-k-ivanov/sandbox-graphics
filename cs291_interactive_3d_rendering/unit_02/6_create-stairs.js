...
function createStairs()
{
    ...
    var yShift = verticalStepHeight + stepThickness
    var zShift = horizontalStepDepth - stepThickness
    ...
    for (let step = 0; step <= 5; step++)
    {
        ...
        // You can define position as THREE.Vector3(x, y, z) or corrdinate shift:
        stepMesh = new THREE.Mesh(stepVertical, stepMaterialVertical);
        stepMesh.position.x = 0;
        stepMesh.position.y = verticalStepHeight / 2 + yShift * step;
        stepMesh.position.z = 0 + zShift * step;
        scene.add(stepMesh);

        // Horizontal part
        stepMesh = new THREE.Mesh(stepHorizontal, stepMaterialHorizontal);
        stepMesh.position.x = 0;
        stepMesh.position.y = verticalStepHeight + stepHalfThickness + yShift * step;
        stepMesh.position.z = horizontalStepDepth / 2 - stepHalfThickness + zShift * step;
        scene.add(stepMesh);
    }
}
