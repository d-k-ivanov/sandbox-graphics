...
{
    // Load Shaders
    vs = await (await fetch('unit_09/09_vertex.glsl')).text();
    fs = await (await fetch('unit_09/09_fragment.glsl')).text();
...
function render()
{
    ...
    phongBalancedGroundMaterial.uniforms.uFlashRadius.value = effectController.flashRadius;
    phongBalancedMaterial.uniforms.uFlashRadius.value = effectController.flashRadius;
