...
{
    // Load Shaders
    vs = await (await fetch('unit_09/07_vertex.glsl')).text();
    fs = await (await fetch('unit_09/07_fragment.glsl')).text();
...
function fillScene()
{
    ...
    teapot = new THREE.Mesh(new THREE.TeapotGeometry(teapotSize, tessLevel[tess], false, true, false), phongBalancedMaterial);
