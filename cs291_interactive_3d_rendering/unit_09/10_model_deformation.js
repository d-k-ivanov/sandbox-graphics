...
{
    // Load Shaders
    vs = await (await fetch('unit_09/10_vertex.glsl')).text();
    fs = await (await fetch('unit_09/10_fragment.glsl')).text();
...
function setupGui()
{
    ...
    h.add(effectController, "sphereRadius", 0.1, 10.0, 0.1).name("sphere radius");
