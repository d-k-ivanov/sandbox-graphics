...
Vector2 lerp(const double t, const Vector2& vec1, const Vector2& vec2)
{
    return (vec2 - vec1) * static_cast<float>(t) + vec1;
}

Vector2 qerp(const double t, const Vector2& vec0, const Vector2& vec1, const Vector2& vec2, const Vector2& vec3)
{
    const auto t2 = static_cast<float>(t * t);
    const Vector2 a0 = vec3 - vec2 - vec0 + vec1;
    const Vector2 a1 = vec0 - vec1 - a0;
    const Vector2 a2 = vec2 - vec0;
    return a0 * static_cast<float>(t) * t2 + a1 * t2 + a2 * static_cast<float>(t) + vec1;
}
